...
{{-- Page Title --}}
@section('title')
    {{ config('app.name') }} » {{ trans('cortex/tenants::common.managerarea') }} » {{ trans('cortex/testimonials::common.testimonials') }} » {{ $testimonial->exists ? $testimonial->name : trans('cortex/testimonials::common.create_testimonial') }}
@endsection

@push('inline-scripts')
    {!! JsValidator::formRequest(Cortex\Testimonials\Http\Requests\Managerarea\TestimonialFormRequest::class)->selector("#managerarea-testimonials-create-form, #managerarea-testimonials-{$testimonial->getKey()}-update-form") !!}
@endpush
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <div class="nav-tabs-custom">
                @if($testimonial->exists && $currentUser->can('delete', $testimonial)) <div class="pull-right"><a href="#" data-toggle="modal" data-target="#delete-confirmation" data-modal-action="{{ route('managerarea.testimonials.destroy', ['testimonial' => $testimonial]) }}" data-modal-title="{!! trans('cortex/foundation::messages.delete_confirmation_title') !!}" data-modal-body="{!! trans('cortex/foundation::messages.delete_confirmation_body', ['type' => 'testimonial', 'name' => $testimonial->name]) !!}" title="{{ trans('cortex/foundation::common.delete') }}" class="btn btn-default" style="margin: 4px"><i class="fa fa-trash text-danger"></i></a></div> @endif
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="details-tab">
                        ...
                        @if ($testimonial->exists)
                            {{ Form::model($testimonial, ['url' => route('managerarea.testimonials.update', ['testimonial' => $testimonial]), 'method' => 'put', 'id' => "managerarea-testimonials-{$testimonial->getKey()}-update-form"]) }}
                        @else
